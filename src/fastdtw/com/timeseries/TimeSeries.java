...
//               timeValueMap.put(timeReadings.get(timeReadings.size()-1), readings);
   ...
   //My Own Constructor to support my trajectories
   
   public TimeSeries(Trajectory t)
   {
      this();
      
      //may be useful to send this as parameter in the future,
      //now there is no poing cause always first measure is time
      ...
      {
    	  ArrayList pointInfoInList = new ArrayList<>();
    	  pointInfoInList.add(p.getT());
    	  pointInfoInList.add((double) p.getX());
    	  pointInfoInList.add((double) p.getY());
    	  timeReadings.add(new Double(timeReadings.size()));
    	  final TimeSeriesPoint readings = new TimeSeriesPoint(pointInfoInList.subList(firstMeasurement, pointInfoInList.size()));
    	  tsArray.add(readings);
      }
   }  // end constructor
