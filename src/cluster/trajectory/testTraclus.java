...
public class testTraclus {
	...
	public static void main(String[] args) {
		...
		ClusteringMethod method = ClusteringMethod.KMEANS;
	...
	private static void CVRRExperiment(ClusteringMethod method) {
		...
		ArrayList<Trajectory> testTrajectoriesCVRR = testTraclus.generateTestTrajectoriesFromDataSetCVRR(CVRRdatasetName);
		...
		*/
		...
		//Parameter for DTW distance
		//eNeighborhoodParameter = (float) 182; //worked for 8 segments
		//eNeighborhoodParameter = (float) 340;	//for complete trajectories (expensive) and cardinality 3
		eNeighborhoodParameter = (float) 3550;	//for complete trajectories (expensive) and cardinality 1
		minLins = 1;
		cardinalityOfClusters = 1;
		//end of douglas peucker ovewriten parameters for test
		traclus = new Traclus(testTrajectoriesCVRR, eNeighborhoodParameter, minLins, cardinalityOfClusters, epsilonDouglasPeucker, fixNumberPartitionSegment, segmentationMethod);
	...
	private static void compareClusters(ArrayList<Cluster> baselineSet, ArrayList<Cluster> testSet, HashSet<Integer> allTrajectories)
	{
		...
		for(Cluster cb: baselineSet)
		{
			...
			float fMeasure = (purity * coverage)/(purity + coverage);
		...
		methodPurity = methodPurity/baselineSet.size();
		methodCoverage = methodCoverage/baselineSet.size();
		methodAccuracy = methodAccuracy/baselineSet.size();
		methodFMeasure = methodFMeasure/baselineSet.size();
