...
public class DBA {
	...
	public static Trajectory DBATrajectories(Trajectory trCentroid, Trajectory[] sequences) {
		...
		for (Trajectory T : sequences) {
			...
			for (int t = nbTuplesAverageSeq - 1; t >= 0; t--) {
				tupleAssociation[i].add(T.getPoints().get(j));
				switch (pathMatrix[i][j]) {
					case DIAGONAL:
						i = i - 1;
						j = j - 1;
		...
		for (int t = 0; t < centerLength; t++) {
			...
			C[t] = barycenter((tupleAssociation[t].toArray()));
			Trajectory centroid = calculateBarycenterTrajectory(tupleAssociation);
		}
		*/
		
		centroid = calculateBarycenterTrajectory(tupleAssociation);
		return centroid;
	}

	private static Trajectory calculateBarycenterTrajectory(
			ArrayList<Point>[] tupleAssociation) {
		// TODO Auto-generated method stub
		
		ArrayList<Point> points = new ArrayList<Point>();
		
		for(ArrayList<Point> pl:tupleAssociation)
		{
			Point tempPoint = new Point(0,0,pl.get(0).getT());
			float x = 0;
			float y = 0;
			...
			tempPoint.setX(x/pl.size());
			tempPoint.setY(y/pl.size());
			points.add(tempPoint);
		}
		
		Trajectory t = new Trajectory(-1, points);
		return t;
	}
