...
public class LocalitySensitiveHashing {
    ...
    private double dilation = 1d;         // defines how "stretched out" the hash values are.
    
    public int[] hashes = null;
...
// 
    ...
    public int[] generateHashes(double[] vectorToHash) {
        ...
        for (int k = 0; k < numFunctionBundles; k++) { 
            ...
            for (int i = 0; i < vectorToHash.length; i++) {
                product += vectorToHash[i] * hashA[k][i];
            }
     ...
     * @throws IOException
     */
    public double[][] createHashFunctions()
    {
    	double[] tmpB = new double[numFunctionBundles];
    	 
    	//Get an array of scalars, for each of the k functions.
        for (int k = 0; k < numFunctionBundles; k++) {
    ...
    public LocalitySensitiveHashing(int dimensions, int numFunctions, int windowSize)
    {
    	this.dimensions = dimensions;
    	this.binLength = windowSize;
    	this.numFunctionBundles = numFunctions;
    }
